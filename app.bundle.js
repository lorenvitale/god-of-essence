...
function setupNoteSwitcher() {
  const switcher = document.querySelector("[data-note-switcher]");
  if (!switcher) return;
  ...
  function activate(tab, { focus = false } = {}) {
    tabs.forEach((candidate) => {
      ...
      candidate.classList.toggle("note-switcher__tab--active", isActive);
  ...
  function focusRelative(currentTab, offset) {
  ...
  const defaultTab =
    tabs.find((tab) => tab.classList.contains("note-switcher__tab--active")) ||
    tabs[0];
